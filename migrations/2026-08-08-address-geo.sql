...
--
-- Safe to run on a live table: ADD COLUMN IF NOT EXISTS only, no rewrite of existing values,
...
ALTER TABLE store_users_shipping_address
  ...
  ADD COLUMN IF NOT EXISTS delivery_lat DECIMAL(10, 7) NULL AFTER delivery_address,
  ADD COLUMN IF NOT EXISTS delivery_lng DECIMAL(10, 7) NULL AFTER delivery_lat,
  ...
  ADD COLUMN IF NOT EXISTS delivery_house VARCHAR(255) NULL AFTER delivery_lng,
  -- "Home" | "Work" | "Other" — what the saved-address list shows as a chip.
  ADD COLUMN IF NOT EXISTS delivery_label VARCHAR(20) NULL AFTER delivery_house,
  ...
  ADD COLUMN IF NOT EXISTS delivery_formatted VARCHAR(500) NULL AFTER delivery_label,
  ...
  ADD COLUMN IF NOT EXISTS delivery_place_id VARCHAR(255) NULL AFTER delivery_formatted;

-- Lets the panel and any future "orders near X" query filter on a bounding box
-- before doing distance maths.
CREATE INDEX IF NOT EXISTS addr_latlng_idx ON store_users_shipping_address (delivery_lat, delivery_lng);
