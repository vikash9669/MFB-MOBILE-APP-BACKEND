...
const { StoreName, StoreMenu, StoreItems, StoreProductsMenu } = require('../models/menu-relations');
...
const getRestaurants = async (req, res) => {
    try {
        const restaurants = await Restaurants.findAll({
            attributes: ['business_id', 'user_id', 'business_name', 'business_menu_types', 'business_open', 'business_close', 'business_commision']
        });
        res.status(200).json(restaurants);
        console.log("working");
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching store products' });
    }
};
...
const getMenu = async (req, res) => {
    try {
        const stores = await Restaurants.findAll({
            attributes: ['business_id', 'user_id', 'business_name', 'business_menu_types']
        });

        const menuStoreMap = new Map();

        stores.forEach(store => {
            const menuIds = store.business_menu_types.split(',').map(id => parseInt(id.trim()));
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching unique menu names with store details' });
    }
};
...
const getBusinessByMenuId = async (req, res) => {
    ...
    try {
        ...
        const menuIds = business.business_menu_types.split(',').map(id => parseInt(id.trim()));
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching business details by menu id' });
    }
};
...
const getMenuForBusinesses = async (req, res) => {
    try {
        ...
        const businesses = await Restaurants.findAll({
            attributes: ['business_id', 'user_id', 'business_name', 'business_offer_text', 'business_open', 'business_close', 'business_status', 'business_discount', 'business_slug', 'business_menu_types']
        });
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching menus for businesses' });
    }
};
...
const getProductNamesByMenuUserId = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching menu names' });
    }
};
...
const getRestaurantDetailsByRestaurantId = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while fetching restaurant details' });
    }
};
